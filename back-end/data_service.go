...
func main() {
	...
	r.HandleFunc("/cards/{card_num}", get_card_info(db)).Methods("GET")

	r.HandleFunc("/cards/market/{id}", get_card_market_data(db)).Methods("GET")
...
func get_card_market_data(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rows, err := db.Query("select id, market_price, date from card_market_data where id = ?", card_id)
		if err != nil {
			log.Fatal(err)
		}
		defer rows.Close()
